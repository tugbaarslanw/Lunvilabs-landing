...
(function () {
  ...
  function openModal(plan){
    ...
    setTimeout(()=>first?.focus(),40); document.documentElement.style.overflow='hidden';
  }
  function closeModal(){
...
(function(){
  const send=(name,label)=>window.gtag&&gtag('event',name,{event_category:'cta',event_label:label});
  document.querySelectorAll('.js-request').forEach(a=>a.addEventListener('click',()=>send('cta_click','request_'+(a.getAttribute('data-plan')||'unknown'))));
  document.querySelectorAll('a.btn-primary, a.btn-ghost').forEach(a=>a.addEventListener('click',()=>send('cta_click',a.textContent.trim())));
})();

// Form UX: disable submit button briefly
(function(){
...
(function(){
  var KEY='cmv2';
  function safeGet(k){try{var v=localStorage.getItem(k);if(v)return v;}catch(e){}var m=document.cookie.match(new RegExp('(?:^|; )'+k+'=([^;]+)'));return m&&m[1];}
  function safeSet(k,val){try{localStorage.setItem(k,val);}catch(e){}document.cookie=k+'='+val+'; Max-Age=31536000; Path=/; SameSite=Lax';}
  function updateConsent(state){var m=(state==='granted')?'granted':'denied'; if(typeof window.gtag==='function'){gtag('consent','update',{ad_storage:m,analytics_storage:m});}}
  ...
  function wire(el){
    ...
    requestAnimationFrame(()=>{try{el.style.position='fixed';el.style.left='16px';el.style.bottom='16px';el.style.zIndex='1200';el.style.maxWidth='520px';}catch(_){}})
  }
  ...
  if(stored){updateConsent(stored); if(existing) destroy(existing);} else if(existing){wire(existing);}
